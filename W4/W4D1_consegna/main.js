...
function inputSearchTitle() {
    //console.log("funzionesearchTitle chiamata")
    const searchTitle = inputTitle.value.toLowerCase()
    //console.log("searchtitle" + searchTitle)

    for (let i = 0; i < jobs.length; i++) {

        const titles = jobs[i].title.toLowerCase()

        if (titles.includes(searchTitle)) {
            arrayTitle.push(jobs[i]);
            countTitle++;

            //console.log("Title: " + jobs[i].title)
        }
    }
    //console.log("array" + arrayTitle)
...
function inputSearchLocation() {
    ...
    console.log("inputlocation" + searchLocation)

    for (let i = 0; i < jobs.length; i++) {

        const locations = jobs[i].location.toLowerCase()
...
function showResult() {
    ...
    for (let i = 0; i < jobs.length; i++) {
        ...
        li.textContent = jobs[i].title + jobs[i].location
        ul.appendChild(li)
    }
...
searchBtn.addEventListener("click", () => {
    ...
    if (titleValue !== "") {
        arrayResult = inputSearchTitle(), showResult()
    } else if (locationValue !== "") {
        arrayResult = inputSearchLocation()
    } else {
        showResult()
    }
